...
**Dataset key:** `lol_v2_synthetic`  
**Generated:** 2026-08-13T05:25:02
...
The paired Train folders contained 900 matched Low/Normal images. The fixed split assigned 810 pairs to optimization and 90 pairs to validation. A deterministic, filename-distributed sample of 256 pairs (maximum 256) was analyzed using thumbnails bounded to 256 pixels.
...
|---|---:|---:|---:|---:|
| Target/input luminance gain | median | 2.4186 | 1.2212 | 4.1221 |
| Dark-pixel ratio (Y < 0.15) | mean | 0.5481 | 0.1301 | 0.8521 |
| Shadow-pixel ratio (Y < 0.25) | mean | 0.6530 | 0.2492 | 0.9274 |
| Dark-region noise proxy | mean | 0.0126 | 0.0027 | 0.0249 |
| Normalized color-balance shift | mean | 0.0507 | 0.0060 | 0.1143 |
| Target chroma strength | mean | 0.0677 | 0.0234 | 0.1168 |
| Target/input detail gain | median | 1.5441 | 0.9534 | 2.7219 |
| Target edge strength | mean | 0.0323 | 0.0136 | 0.0514 |
| Spatial darkness variation | mean | 0.1491 | 0.0439 | 0.2911 |
| Target highlight clipping ratio | mean | 0.0239 | 0.0000 | 0.0755 |
...
|---|---|---|---:|---|
| Severe enhancement | gain=2.4186; dark ratio=0.5481 | gain >= 4.00 OR dark ratio >= 0.60 | not activated | Controls the LR interval and gradient-clipping candidates. |
| Large training set | training pairs=810 | training pairs >= 4000 | not activated | Selects a more conservative LR interval when activated. |
| Noisy low-light data | noise proxy=0.0126 | noise proxy >= 0.018 | not activated | Expands conservative optimization and noise-penalty ranges. |
| Structural fidelity demand | detail gain=1.5441; target edge=0.0323 | detail gain >= 1.10 OR target edge >= 0.035 | activated | Controls the W_SSIM and W_LPIPS domains. |
| Color/chroma search | color shift=0.0507; chroma=0.0677 | color shift >= 0.025 OR target chroma >= 0.075 | activated | Activates W_COLOR and W_CHROMA. |
| Edge-loss search | detail gain=1.5441; target edge=0.0323 | detail gain >= 1.05 OR target edge >= 0.030 | activated | Activates W_EDGE. |
| Dark-noise-loss search | noise proxy=0.0126; dark ratio=0.5481 | noise proxy >= 0.012 OR dark ratio >= 0.40 | activated | Activates W_DARK_NOISE. |
| Attention-regularization search | dark ratio=0.5481; spatial variation=0.1491 | dark ratio >= 0.35 OR spatial variation >= 0.08 | activated | Activates W_ATTN. |
...
| illumination | `LAMBDA_MAX` | 1.35 | 1.05 to 1.7 (uniform), step=0.05 | The required brightness gain is mild; a lower amplification range reduces the risk of over-enhancement. |
...
|---|---:|---:|---:|---:|
| optimizer | 10 | 20 | 0 | 20 |
| illumination | 10 | 25 | 20 | 5 |
| fidelity | 12 | 30 | 25 | 5 |
| detail | 14 | 35 | 30 | 5 |
...
For LOL-v2 / Synthetic, the hyperparameter search space was constructed from prespecified rules applied exclusively to the training partition. The analysis measured target-to-input luminance gain (2.4186), dark-pixel coverage (0.5481), a dark-region high-frequency noise proxy (0.0126), normalized color shift (0.0507), detail gain (1.5441), target edge strength (0.0323), and spatial darkness variation (0.1491). These measurements activated dataset-relevant parameter groups and constrained their candidate domains, while architecture-defining and controlled loss parameters remained fixed at the source-profile values. Candidate configurations were compared on a fixed validation split using PSNR, SSIM, and LPIPS, with the Test partition reserved for post-selection evaluation.
