...
**Dataset key:** `lol_v2_real`  
**Generated:** 2026-08-12T14:28:14
...
The paired Train folders contained 689 matched Low/Normal images. The fixed split assigned 620 pairs to optimization and 69 pairs to validation. A deterministic, filename-distributed sample of 256 pairs (maximum 256) was analyzed using thumbnails bounded to 256 pixels.
...
|---|---:|---:|---:|---:|
| Target/input luminance gain | median | 7.7078 | 3.7330 | 24.2346 |
| Dark-pixel ratio (Y < 0.15) | mean | 0.9324 | 0.7634 | 1.0000 |
| Shadow-pixel ratio (Y < 0.25) | mean | 0.9871 | 0.9769 | 1.0000 |
| Dark-region noise proxy | mean | 0.0050 | 0.0008 | 0.0119 |
| Normalized color-balance shift | mean | 0.1138 | 0.0133 | 0.2406 |
| Target chroma strength | mean | 0.0495 | 0.0193 | 0.0888 |
| Target/input detail gain | median | 3.9914 | 1.3846 | 7.9437 |
| Target edge strength | mean | 0.0229 | 0.0125 | 0.0352 |
| Spatial darkness variation | mean | 0.0209 | 0.0083 | 0.0379 |
| Target highlight clipping ratio | mean | 0.0029 | 0.0000 | 0.0033 |
...
|---|---|---|---:|---|
| Severe enhancement | gain=7.7078; dark ratio=0.9324 | gain >= 4.00 OR dark ratio >= 0.60 | activated | Controls the LR interval and gradient-clipping candidates. |
| Large training set | training pairs=620 | training pairs >= 4000 | not activated | Selects a more conservative LR interval when activated. |
| Noisy low-light data | noise proxy=0.0050 | noise proxy >= 0.018 | not activated | Expands conservative optimization and noise-penalty ranges. |
| Structural fidelity demand | detail gain=3.9914; target edge=0.0229 | detail gain >= 1.10 OR target edge >= 0.035 | activated | Controls the W_SSIM and W_LPIPS domains. |
| Color/chroma search | color shift=0.1138; chroma=0.0495 | color shift >= 0.025 OR target chroma >= 0.075 | activated | Activates W_COLOR and W_CHROMA. |
| Edge-loss search | detail gain=3.9914; target edge=0.0229 | detail gain >= 1.05 OR target edge >= 0.030 | activated | Activates W_EDGE. |
| Dark-noise-loss search | noise proxy=0.0050; dark ratio=0.9324 | noise proxy >= 0.012 OR dark ratio >= 0.40 | activated | Activates W_DARK_NOISE. |
| Attention-regularization search | dark ratio=0.9324; spatial variation=0.0209 | dark ratio >= 0.35 OR spatial variation >= 0.08 | activated | Activates W_ATTN. |
...
For LOL-v2 / Real_captured, the hyperparameter search space was constructed from prespecified rules applied exclusively to the training partition. The analysis measured target-to-input luminance gain (7.7078), dark-pixel coverage (0.9324), a dark-region high-frequency noise proxy (0.0050), normalized color shift (0.1138), detail gain (3.9914), target edge strength (0.0229), and spatial darkness variation (0.0209). These measurements activated dataset-relevant parameter groups and constrained their candidate domains, while architecture-defining and controlled loss parameters remained fixed at the source-profile values. Candidate configurations were compared on a fixed validation split using PSNR, SSIM, and LPIPS, with the Test partition reserved for post-selection evaluation.
